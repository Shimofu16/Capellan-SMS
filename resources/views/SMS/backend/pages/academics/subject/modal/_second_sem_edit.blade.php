<div class="modal fade" id="edit{{ $secondSemSubject->id }}" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.academic.subject.update', ['id' => $secondSemSubject->id]) }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-12">
                            ...
                            <select name="type" id="type" class="form-control">
                                ...
                                <option value="Core" @if ($secondSemSubject->type == 'Core') selected @endif>Core</option>
                                <option value="Applied and Specialized Subjects" @if ($secondSemSubject->type == 'Applied and Specialized Subjects') selected @endif>Applied and Specialized Subjects
                                </option>
                            </select>
                            @error('type')
                                <div class="text-danger">{{ $message }}</div>
                            ...
                            <select name="grade_level_id" id="grade_level_id" class="form-control">
                                ...
                                @foreach ($gradeLevels as $gradLevel)
                                    <option value="{{ $gradLevel->id }}"
                                        @if ($gradLevel->id == $secondSemSubject->grade_level_id) selected @endif>
                                        {{ $gradLevel->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="specialization_id" id="specialization_id" class="form-control">
                                ...
                                @foreach ($specializations as $specialization)
                                    <option value="{{ $specialization->id }}"
                                        @if ($specialization->id == $secondSemSubject->specialization_id) selected @endif>
                            ...
                            <select name="semester_id" id="semester_id" class="form-control">
                                ...
                                @foreach ($semesters as $semester)
                                    <option value="{{ $semester->id }}"
                                        @if ($semester->id == $secondSemSubject->semester_id) selected @endif>{{ $semester->sem }}
                                    </option>
                                @endforeach
                            </select>
                            @error('semester_id')

    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (auth()->user()->status == "online") {
            ...
            if (time() - $last_activity_timestamp > 600) {
                ...
                return redirect()->route('auth.index')->with('errorAlert', 'Your account is inactive due to inactivity. Please login again.');
            }
        ...
        if (auth()->user()->status == "inactive") {
            return redirect()->route('auth.index')->with('errorAlert', 'Your account is inactive. Please contact the administrator.');
        }
        if (auth()->user()->status == "offline") {
            return redirect()->route('auth.index')->with('errorAlert', 'Your account is offline due to inactive or force logout.');
        }
        $request->session()->forget(Auth::id() . "_last_activity");
